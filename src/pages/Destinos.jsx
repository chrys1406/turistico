...
export default function Destinos() {
  ...
  return (
    <div style={{ fontFamily: "'Inter', sans-serif", background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background: '#1a1a2e' }} className="pt-24 md:pt-32 pb-10 md:pb-16 px-4 md:px-12">
        <div className="max-w-[1440px] mx-auto">
          <p className="text-xs font-bold tracking-[0.3em] text-amber-400 uppercase mb-3">
            Explora La Paz & El Alto
          </p>
          <h1
            className="text-3xl md:text-5xl lg:text-6xl font-black text-white mb-3 md:mb-4"
          ...
          {/* Buscador */}
          <div className="mt-6 md:mt-8 flex gap-2 md:gap-3 max-w-2xl">
            <div className="flex-1 relative">
              <Search size={15} className="absolute left-3 md:left-4 top-1/2 -translate-y-1/2 text-slate-400" />
              <input
                type="text"
                placeholder="Buscar destino..."
                value={busqueda}
                onChange={e => setBusqueda(e.target.value)}
                className="w-full pl-9 md:pl-11 pr-4 py-3 md:py-3.5 rounded-xl text-sm font-medium outline-none transition-all"
                style={{
            ...
            <button
              ...
              className="flex items-center gap-1.5 md:gap-2 px-3 md:px-4 py-3 rounded-xl text-xs md:text-sm font-semibold transition-all shrink-0"
              style={{
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {CATEGORIAS.map(cat => (
                <button
                  ...
                  className="px-3 md:px-4 py-1.5 md:py-2 rounded-full text-xs font-bold uppercase tracking-wider transition-all"
                  style={{
      ...
      {/* ── RESULTADOS ── */}
      <div className="max-w-[1440px] mx-auto px-4 md:px-12 py-8 md:py-12">
            ...
            {destinosFiltrados.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                {destinosFiltrados.map(d => (
                  <div
                    key={d.id}
                    className="bg-white rounded-2xl overflow-hidden shadow-sm hover:-translate-y-1 md:hover:-translate-y-2 transition-all duration-300 group cursor-pointer flex flex-col"
                  ...
                  >
                    ...
                    <div className="p-4 md:p-5 flex-1 flex flex-col">
                      <div className="flex items-start justify-between mb-2">
                        <h3
                          className="font-black text-base md:text-lg text-slate-900 leading-tight"
                          style={{ fontFamily: "'Playfair Display', serif" }}
                        >
                          {d.nombre}
                        </h3>
                      ...
                      <p className="text-slate-500 text-xs md:text-sm leading-relaxed mb-4 flex-1 line-clamp-3">
                        {d.descripcion}
                      </p>

                      <div className="flex items-center justify-between pt-3 md:pt-4" style={{ borderTop: '1px solid #f1f5f9' }}>
              ...
              <div className="text-center py-16 md:py-24 px-4">
                ...
                <h3
                  className="text-xl md:text-2xl font-black text-slate-700 mb-2"
                  style={{ fontFamily: "'Playfair Display', serif" }}
                >
                  Sin resultados
                </h3>
